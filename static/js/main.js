...
const TagManager = {
    ...
    debouncedGenerate: debounce(async function() {
        ...
        try {
            ...
            // Check if we have too many tags and suggest batch processing
            if (checkedTags.length > 100) {
                console.warn(`Large tag set detected (${checkedTags.length} tags). Consider reducing selection to prevent timeouts.`);
            }
            ...
            if (!response.ok) {
                ...
                if (response.status === 408) {
                    ...
                    errorMessage = 'Rate limit exceeded. Please wait a moment before generating labels again.';
                } else {
                    try {
                        const error = await response.json();
            ...
            if (error.message.includes('timed out') || error.message.includes('502')) {
                userMessage = `Generation failed due to timeout. You selected ${checkedTags.length} tags. 
                
To fix this:
1. Try selecting fewer tags (50 or less recommended)
2. Use filters to narrow down your selection
3. Generate labels in smaller batches

Current selection: ${checkedTags.length} tags`;
            }
