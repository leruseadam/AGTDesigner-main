...
"""
Robust fix for _calculate_product_strain method signature mismatch.
Adds an overloaded version that accepts product_data dictionary with proper indentation.
"""
...
def fix_method_signature():
    ...
    # Find the end of the ProductDatabase class by looking for the last method
    lines = content.split('\n')
    insert_index = -1
    
    # Look for the last method definition in the class
    # We'll look for methods that are indented with 4 spaces (class methods)
    for i in range(len(lines) - 1, -1, -1):
        line = lines[i].strip()
        if line.startswith('def ') and line.count('    ') >= 1:  # At least one level of indentation
    ...
    # Create the overloaded method with proper indentation
    overloaded_method_lines = [
    ...
    for j, method_line in enumerate(overloaded_method_lines):
        lines.insert(insert_index + j, method_line)
    ...
    # Test the syntax by trying to parse the file
    try:
        with open(file_path, 'r') as f:
            ast.parse(f.read())
